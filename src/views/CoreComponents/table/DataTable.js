...
const CDataTable = (props) => {
  ...
  const [filterByDay, setFilterByDays] = useState("");
  ...
  const dayFilterChange = (e) => {
    onDayFilterChange && onDayFilterChange(Number(e.target.value));
    setFilterByDays(Number(e.target.value));
  };
  ...
  const tableFilterData = {
    label: (tableFilter && tableFilter.label) || "Search:",
    placeholder: (tableFilter && tableFilter.placeholder) || "",
  };
  ...
  return (
    <React.Fragment>
      ...
      {pagination && (
        <CPagination
          {...paginationProps}
          style={{ display: totalPages > 1 ? "inline" : "none", width: "100%" }}
